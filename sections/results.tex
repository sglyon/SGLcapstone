...
\subsection{hsfpy Usage} \label{sub:hsfpy_usage}
  ...
  Although \texttt{hsfpy} is not entirely finished, there is still a usable subset of \texttt{hsf} available at the time of writing. The basic data structure in \texttt{hsf} is the hierarchical spline forest, which is implemented as \texttt{HForest} and has already been wrapped in \texttt{hsfpy}.
